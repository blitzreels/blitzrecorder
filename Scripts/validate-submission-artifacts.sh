...
validate_export_options_plist() {
  ...
  validate_plist_value "$label export options" "$plist" "signingStyle" "automatic"
...
validate_archived_app() {
  ...
  if [[ "$label" == "iOS" ]]; then
    ...
    if [[ "$capabilities" == *"camera"* ]]; then
      echo "✓ iOS archived app requires camera capability"
    else
      fail "iOS archived app UIRequiredDeviceCapabilities is ${capabilities:-missing}, expected camera"
    fi
