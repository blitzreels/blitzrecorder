...
capture_simulator() {
  ...
  # Marketing screenshots are always dark (landing/brand theme).
...
capture_ipad() {
  # Capture the real UI to a raw/ subfolder, then compose the branded
  # marketing screenshot into the upload folder (same flow as iPhone).
  local raw_dir="$IPAD_OUTPUT_DIR/raw"
  capture_simulator \
    "iPad 13-inch" \
    "$raw_dir" \
    "${IPAD_SIMULATOR_NAME:-BlitzRecorder iPad 13}" \
    ...
    "2048x2732 2064x2752" \
    "01-pairing-screen:pairing"
  swift "$ROOT/Scripts/compose-app-store-screenshots.swift" "$raw_dir" "$IPAD_OUTPUT_DIR" 2064x2752 ipad
}
