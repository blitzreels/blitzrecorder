...
final class RemoteIPhoneCameraSessionTests: XCTestCase {
    ...
    func testFreshAutomaticRotationTelemetryCreatesSavedRemoteRotation() async {
        let service = makeService(id: "Alice-iPhone._blitzrecorder-camera._tcp.local.", name: "Alice iPhone")
        var settings = RecordingSettings()
        settings.selectedCameraID = RemoteCameraProviderID.make(for: service.id)
        let browser = FakeRemoteIPhoneCameraBrowser()
        ...
        let savedSettings = settings.remoteCameraSettingsByServiceID[service.id]
        XCTAssertEqual(savedSettings?.rotationDegrees, 90)
        XCTAssertTrue(savedSettings?.usesAutomaticRotation ?? false)
    }
    ...
    func testRecordingRotationTelemetryDoesNotRewriteSavedRemoteRotation() async {
        let service = makeService(id: "Alice-iPhone._blitzrecorder-camera._tcp.local.", name: "Alice iPhone")
        var settings = RecordingSettings()
        settings.selectedCameraID = RemoteCameraProviderID.make(for: service.id)
        settings.remoteCameraSettingsByServiceID[service.id] = RemoteCameraSettings(
        ...
        browser.publish(services: [service])
        await Task.yield()
        control.publish(.telemetry(makeTelemetry(phase: .recording, rotationDegrees: 90)))
        await Task.yield()
    ...
    private func makeTelemetry(
        phase: RemoteCameraRecordingPhase = .idle,
        rotationDegrees: Int
