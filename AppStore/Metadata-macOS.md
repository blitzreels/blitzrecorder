...
- User defaults: Used for app settings, export quota, and trusted device IDs
- File metadata: Used for app-created recordings and user-selected recording/export locations
- App Store purchases: Handled by Apple StoreKit
- BlitzReels entitlement check: Stores the BlitzReels access token in the macOS Keychain and sends it to BlitzReels only when verifying included access
