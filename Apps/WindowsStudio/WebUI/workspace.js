...
let state = {targets: [], takes: [], recording: false, playing: false, paused: false};
let view = 'record';
...
function requestBounds() {
  ...
  send(`bounds|${Math.round(rect.left * scale)}|${Math.round(rect.top * scale)}|${Math.round(rect.width * scale)}|${Math.round(rect.height * scale)}`);
}
...
function render() {
  ...
  $('record-label').textContent = state.recording ? 'Stop recording' : state.starting ? 'Starting…' : 'Start recording';
  ...
  for (const [key, element] of [['microphone','microphone-switch'],['systemAudio','system-switch'],['camera','camera-switch']]) {
    $(element).classList.toggle('on', !!state[key]);
  }
  const selectedTarget = state.targets[state.selected] || 'Choose a screen or window';
  ...
  if (select.dataset.names !== targetNames) {
    select.replaceChildren(...state.targets.map((label, index) => new Option(label, String(index))));
    select.dataset.names = targetNames;
  }
  select.value = String(state.selected);
  select.disabled = busy;
  $('last-take').textContent = state.lastTake ? state.lastTake.split(/[\\/]/).pop() : 'Record a take to preview and export it here.';
  $('scene-title').textContent = state.camera ? 'Camera overlay' : 'Screen focus';
  ...
  if (state.takes.length) {
    list.replaceChildren(...state.takes.map((name, index) => {
      ...
      card.addEventListener('click', () => { send(`take|${index}`); setView('record'); });
      return card;
    }));
  } else {
    const empty = document.createElement('div');
    empty.className = 'empty-library';
    empty.textContent = 'Your recordings will appear here after you finish a take.';
...
$('library-button').addEventListener('click', () => { send('refresh'); setView('library'); });
...
$('record-button').addEventListener('click', () => send(state.recording ? 'stop' : 'start'));
...
$('target-select').addEventListener('change', (event) => send(`target|${event.target.value}`));
new ResizeObserver(requestBounds).observe($('preview-frame'));
window.addEventListener('resize', requestBounds);
window.addEventListener('keydown', (event) => {
  if (event.code === 'Space' && event.target.tagName !== 'SELECT' && event.target.tagName !== 'BUTTON') {
