...
struct CameraCaptureSettingsPlanner {
    ...
    func preserveActiveCaptureSettings(
        ...
        preservedSettings.colorMode = activeSettings.colorMode
        preservedSettings.stabilizationMode = activeSettings.stabilizationMode
        preservedSettings.usesAutomaticRotation = activeSettings.usesAutomaticRotation
        preservedSettings.rotationDegrees = activeSettings.rotationDegrees
    ...
    private func normalizedCinematicCaptureFormat(
        ...
        let profiles = RemoteCameraCaptureProfileResolver.supportedProfiles(
            for: activeDevice,
            movieOutput: capabilityBuilder.movieOutput
        )
        normalizedSettings.captureProfileID = RemoteCameraSettingsResolver.preferredCinematicCaptureProfile(
            from: profiles
        )
