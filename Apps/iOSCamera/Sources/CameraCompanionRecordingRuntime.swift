...
final class CameraCompanionRecordingRuntime {
    ...
    typealias StartCameraRecording = @MainActor (_ takeID: UUID) async throws -> URL
    ...
    private func startRecording(timeline: RemoteCameraTimeline) {
        ...
        startRecordingTask = Task {
            do {
                ...
                let recordingURL = try await startCameraRecording(timeline.takeID)
                guard !Task.isCancelled,
                ...
                activeRecordingURL = recordingURL
                activeStopReason = nil
                let deviceStartTime = DispatchTime.now().uptimeNanoseconds
