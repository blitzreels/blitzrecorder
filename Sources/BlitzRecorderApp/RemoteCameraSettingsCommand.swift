...
enum RemoteCameraSettingsCommand {
    ...
    private static func applyCinematicDefaults(
        ...
        settings.captureProfileID = RemoteCameraSettingsResolver.preferredCinematicCaptureProfile(
            from: lensCapabilities?.supportedCaptureProfiles ?? []
        )
        if let cinematicFormat = preferredCinematicFormat(from: lensCapabilities?.supportedFormats) {
            settings.formatID = cinematicFormat.id
            settings.frameRate = cinematicFormat.frameRates.contains(30)
                ? 30
                : (cinematicFormat.frameRates.first ?? settings.frameRate)
        }
    ...
    private static func preferredCinematicFormat(from formats: [RemoteCameraFormat]?) -> RemoteCameraFormat? {
        guard let formats else { return nil }
        let cinematicFormats = formats.filter(\.supportsCinematicVideo)
        let candidates = cinematicFormats.isEmpty ? formats : cinematicFormats
        return candidates
            .filter { $0.frameRates.contains(30) }
            .max(by: preferHigherQualityFormat)
            ?? candidates.max(by: preferHigherQualityFormat)
    }

    private static func preferHigherQualityFormat(_ lhs: RemoteCameraFormat, _ rhs: RemoteCameraFormat) -> Bool {
