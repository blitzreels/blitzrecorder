...
function assertPaidBlitzRecorderSession(session: Stripe.Checkout.Session): void {
  ...
  if (!sessionPriceIds(session).includes(BLITZRECORDER_EARLY_PRICE_ID)) {
    throw new Error("Checkout session is not for BlitzRecorder Early Price");
  }
}

export async function claimLicenseForCheckoutSession(sessionId: string): Promise<ClaimedLicense> {
  ...
  if (isLicenseStoreConfigured()) {
    await upsertLicense(payload).catch(() => {});
  }
