...
/** Direct DMG when a release is known, else GitHub's latest-release page. */
function downloadHref(dmgUrl: string | undefined): string {
  return dmgUrl ?? LATEST_RELEASE_URL;
}

/**
 * Primary CTA. Always offers the macOS download: the exact DMG once a release
 * exists, otherwise GitHub's `/releases/latest`. Callers own sizing via className.
 */
export function DownloadButton({
  ...
  return (
    <Button variant={variant} render={<a href={downloadHref(release?.dmgUrl)} />} className={className}>
      <Download />
      {label}
    </Button>
  );
...
/** `vX.Y.Z` linking to the release; falls back to the current app version. */
export function VersionTag({ className }: { className?: string }) {
  const release = useRelease();
  const version = release?.version ?? FALLBACK_VERSION;
  return (
    <a href={release?.htmlUrl ?? RELEASES_URL} target="_blank" rel="noopener" className={className}>
      v{version}
    </a>
  );
}
